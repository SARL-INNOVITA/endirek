...
|---|---|---|---|---|
| **Base de données** (PostgreSQL/PostGIS) | Docker/PostGIS local validé, mais driver repositories postgres de l'API non implémenté | `DB_DRIVER=mock` : adapter **implémenté (étape 2 faite)** derrière les mêmes interfaces de repositories que le futur driver PostgreSQL ; données en mémoire, seed La Réunion chargé au boot si `DB_MOCK_SEED=true` (défaut) — schéma et décisions : [DATABASE.md](DATABASE.md) | `DB_DRIVER`, `DB_MOCK_SEED`, `DATABASE_URL`, `POSTGRES_HOST`, `POSTGRES_PORT`, `POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD` | Implémenter les repositories SQL puis seulement passer `DB_DRIVER=postgres` + `DATABASE_URL` (procédure : [DATABASE.md](DATABASE.md) §7) ; en prod : base managée ou serveur Hetzner |
