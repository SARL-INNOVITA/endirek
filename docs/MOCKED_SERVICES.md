...
|---|---|---|---|---|
| **Base de données** (PostgreSQL/PostGIS) | Docker non installé sur la machine de dev, pas de base managée fournie | `DB_DRIVER=mock` : adapter local (implémenté à l'étape 2) derrière la même interface que le driver PostgreSQL ; données en mémoire + seed La Réunion | `DB_DRIVER`, `DATABASE_URL`, `POSTGRES_HOST`, `POSTGRES_PORT`, `POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD` | Dès que Docker est installé : `infra/docker-compose.yml` puis `DB_DRIVER=postgres` + `DATABASE_URL` ; en prod : base managée ou serveur Hetzner |
...
| **Email Brevo** | Compte Brevo non fourni | `EMAIL_DRIVER=mock` : les emails (vérification, reset mot de passe) sont **logués en console** au lieu d'être envoyés — le contenu est visible dans les logs de l'API | `EMAIL_DRIVER`, `BREVO_API_KEY`, `BREVO_SENDER_EMAIL`, `BREVO_SENDER_NAME` | Fin Lot 1 / production : fournir les clés Brevo, `EMAIL_DRIVER=brevo` |
| **OAuth Google / Apple** | Clients OAuth non configurés | Boutons **désactivés** dans l'app mobile (avec mention « bientôt disponible ») ; endpoints API présents mais répondant **`501 Not Implemented`** ; l'auth email/mot de passe (étape 3) couvre le besoin | `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `APPLE_CLIENT_ID` | Lot 2 (ou fin Lot 1 si les clés arrivent) : renseigner les variables, implémenter la vérification de token derrière les endpoints existants, activer les boutons |
