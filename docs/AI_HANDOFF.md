...
_Dernière mise à jour : Lot 2 — CP2.1→CP2.4 validés et poussés + **CP2.5 implémenté** (modération avancée Dealplace — D65/D66/D67, validation product owner à venir) : le Lot 2 est COMPLET (2026-07-12)._
...
| **Lot 2 — Dealplace** | Marketplace biens/services, annonces, conversations 1-to-1 temps réel, deals contractuels (états, éléments validables, litiges), modération avancée | **COMPLET — CP2.1→CP2.4 validés et poussés ; CP2.5 (modération avancée) implémenté, validation product owner attendue** |
...
**CP2.5 (modération avancée) validé et POUSSÉ le 2026-07-13** (`bd59d73` feat + `3f06785` doc). **Au-dessus, un correctif LOCAL non poussé** (dernier commit, `fix: carte — contrainte de caméra containCenter` — voir `git log`) : crash `flutter_map` sur l'onglet Carte en portrait (voir §6), en attente de re-test product owner sur émulateur avant push.
Branche : `main`. Historique : CP2.1→CP2.4 poussés (2026-07-11/12), CP2.5 poussé (2026-07-13). **Le Lot 2 est complet et intégralement poussé** ; seul le correctif carte reste local.
...
Fonctionnelle. Modules livrés : `health`, `database` (mock/postgres), `auth`, `users`, `admin` (utilisateurs + posts + signalements + caméras + types de posts + commentaires signalés + notifications système dev/mock + **annonces & taxonomie Dealplace** + **deals/litiges & conversations — CP2.5**), `media`, `posts`, `feed`, `comments`, `reactions`, `saved-posts`, `moderation` (posts + **annonces — CP2.5**), `map`, `cameras`, `notifications`, `realtime`, **`dealplace` (Lot 2 — CP2.1/CP2.2)**, **`conversations` (Lot 2 — CP2.3/CP2.5, remplace `_future/conversations`)**, **`deals` (Lot 2 — CP2.4/CP2.5, remplace `_future/deals`)**.
...
(`postgis/postgis:16-3.4`) ; migrations `0001`→`0008` appliquées avec succès
(`0001`/`0002` Lot 1, `0003`/`0004` CP2.1, `0005` CP2.2, `0006` CP2.3,
`0007` CP2.4, `0008` CP2.5 — `npm run db:migrate` applique tout le dossier
...
**Log de boot attendu (mock)** : `Mock DB prête : 15 utilisateurs, 32 follows, 42 posts (dont 13 visibles carte), 60 commentaires, 155 réactions, 12 caméras, 5 signalements, 12 notifications, 8 annonces Dealplace (20 catégories, 79 sous-catégories, 10 tags), 3 conversations (9 messages), 3 deals (2 avis)` (compteurs enrichis au CP2.5 : +1 signalement d'annonce, +1 conversation/3 messages, +1 deal en litige).
**Log de boot attendu (postgres, première base seedée)** : `PostgreSQL prêt : connecté (15 utilisateurs, 32 follows, 42 posts (dont 13 visibles carte), 60 commentaires, 155 réactions, 12 caméras, 5 signalements, 12 notifications)` (la ligne postgres s'arrête aux tables du Lot 1 — asymétrie assumée avec le mock).
...
**Lot 1 terminé (7 checkpoints + Lot 1.5), Lot 2 COMPLET (CP2.1 → CP2.5).**

**Le CP2.5 est implémenté et vérifié** (builds/tests verts — 43 tests
flutter ; sondes croisées mock/postgres STRICTEMENT identiques couvrant :
signalement d'annonce avec doublon/auto/404, file admin + compteurs, les
TROIS issues d'arbitrage avec re-litige après reprise et avis ouverts après
conclusion arbitrée, erreurs 400/403/409, masquage/réactivation de message
avec corps remplacé et fil intact) mais **NON poussé** : attendre la
**validation du product owner du CP2.5**, pousser, puis **arbitrer la
suite** — le Lot 2 est complet ; candidats : validation product owner du
Lot 1 (checkpoint 7 toujours en attente), Lot 3 (pages restaurants /
entreprises) ou dettes transverses (tests e2e API, rate limiting,
vérification d'email — voir [KNOWN_LIMITS.md](KNOWN_LIMITS.md)).
...
2. **Rester dans le périmètre du checkpoint courant.** Le Lot 2 est COMPLET (CP2.1 → CP2.5, ce dernier en attente de validation). Ne développe PAS, avant le feu vert du product owner : pages restos/entreprises (Lot 3), News IA (Lot 4), premium/Google Ads réel, arbitrage IA des litiges. **Paiement = hors app.**
...
6. **Ne pas créer les tables des lots non encore démarrés** (pages, news, billing…) ; se contenter de les documenter. Toutes les tables du Lot 2 sont créées (migrations 0003→0008 — listings/taxonomie, `dealplace_seeking`, conversations/messages, deals, modération).
