...
_Dernière mise à jour : Lot 2 — CP2.1/CP2.2 validés et poussés + **CP2.3 implémenté** (conversations 1-to-1 temps réel — D63, validation product owner à venir) (2026-07-11)._
...
**Dernier commit connu : `b1258e8`** — `feat: Conversations 1-to-1 temps réel liées aux annonces (Lot 2 CP2.3)`.
Branche : `main`. Historique récent : `30581dd` (**CP2.2 — Profil Dealplace**) → `1ca0a79` (doc CP2.2) → `1a8f0e6` (fix export RGPD : bloc `listings`) → `44dfb24`/`5f45a0c` (docs validation CP2.2 — poussés) → `b1258e8` (**CP2.3 — Conversations**, **local, non poussé** : validation product owner attendue). CP2.1 et CP2.2 **validés par le product owner et poussés le 2026-07-11**.
...
- **Temps réel** : gateway **socket.io** (namespace par défaut, auth handshake JWT), events `notification.created` (room `user:<id>`), `map.updated` (room `map`) et **`message.created`** (CP2.3 — room `user:<id>` du destinataire, payload `{ conversationId, message, unreadConversations }`), CORS aligné sur l'API via `RealtimeIoAdapter`.
...
**Log de boot attendu (mock)** : `Mock DB prête : 15 utilisateurs, 32 follows, 42 posts (dont 13 visibles carte), 60 commentaires, 155 réactions, 12 caméras, 4 signalements, 12 notifications, 8 annonces Dealplace (20 catégories, 79 sous-catégories, 10 tags), 2 conversations (6 messages)` (suffixe Dealplace ajouté au CP2.1).
...
**Le CP2.3 est implémenté et vérifié** (builds/tests verts, sondes croisées
mock/postgres identiques — 11 contrôles, test bout-en-bout du temps réel
`message.created` réussi) mais **NON poussé** : attendre la **validation du
product owner du CP2.3**, pousser, puis **démarrer le CP2.4** — deals
contractuels (machine à états, éléments validables, litiges) **+ avis
détaillés** (reportés du CP2.2, D59) ; le bouton « Proposer un deal » et les
blocs avis/deals du profil sont les placeholders à activer, et les fils de
conversation (CP2.3) accueilleront la négociation. Ne pas anticiper la
